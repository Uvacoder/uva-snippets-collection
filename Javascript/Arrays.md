...
It deletes elements from the array. It takes 2 parameters. Being the first one the item to delete(index), and the second one how many items to delete after.

```javascript
fruits.splice(0, 1); //meaning that it will start from the first index and end on the secont( not inclusive)

fruits = ["orange", "lemon", "banana", "mango"];
```

 <h4><li>Other Examples</h4></li>

So we have one empty array and we want to put one argument not in the first index but in the..5!!
Why? Just because, but the main thing is it actually possible.
...
(4)
  [("red", "orange", "yellow", "blue")](4)
  [("red", "orange", "yellow", "blue")](4)
  [("red", "orange", "yellow", "blue")](4)[("red", "orange", "yellow", "blue")];
